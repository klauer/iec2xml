...
def production():
    return non_terminal_symbol, "::=", [alternation, concat], ";"


def concat():
    return (
        -2,
        [regex, terminal_symbol, non_terminal_symbol, parentheses, closure, option],
    )


def _extended_structure():
    return [
        alternation,
        regex,
        terminal_symbol,
...
def genPEG(obj, genparens=True):
    ...
    if type(obj) == type([]):
        for e in range(len(obj)):
            ...
            if e == 0:
                if not m and len(obj) > 1 and genparens:
                    result += "("
            result += text
            if e < len(obj) - 1:
                if m:
