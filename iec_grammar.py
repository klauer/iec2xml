...
def integer_literal():
    return (
        ...
        (integer_type_name, "#"),
        re.compile(
            r"(2\#(1|0)(_?(1|0))*)|(8\#[0-7](_?[0-7])*)|(16\#[0-9A-F](_?[0-9A-F])*)|((\+|\-)?[0-9](_?[0-9])*)"
        ),
    )
...
def binary_integer():
    return ("2#", re.compile(r"(1|0)(_?(1|0))*"))


def bit():
    return re.compile(r"1|0")
...
def real_literal():
    return [
        (
            ...
            (real_type_name, "#"),
            re.compile(
                r"((\+|\-)?[0-9](_?[0-9])*)\.([0-9](_?[0-9])*)((e|E)(\+|\-)?([0-9](_?[0-9])*))?"
            ),
        ),
        (
            0,
            (real_type_name, "#"),
            re.compile(r"((\+|\-)?[0-9](_?[0-9])*)((e|E)(\+|\-)?([0-9](_?[0-9])*))"),
        ),
    ]


def exponent():
    return re.compile(r"(E|e)(\+|\-)?[0-9](_?[0-9])*")
...
def _unsigned_integer_type_name():
    return [type_usint, type_uint, type_udint, type_ulint]
...
def bit_string_type_name():
    return [type_bool, type_byte, type_word, type_dword, type_lword]
...
def location_prefix():
    return re.compile(r"I|Q|M")


def size_prefix():
    return re.compile(r"X|B|W|D|L")
...
def incompl_located_var_declarations():
    return (
        keyword("VAR"),
        0,
        [retain, non_retain],
...
def incompl_located_var_decl():
    return (variable_name, incompl_location, ":", var_spec)


def incompl_location():
    return (keyword("AT"), re.compile(r"\%(I|Q|M)\*"))
